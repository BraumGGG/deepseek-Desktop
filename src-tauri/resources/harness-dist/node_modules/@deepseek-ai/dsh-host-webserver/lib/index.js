...
function createGzipMiddleware(config) {
	...
	return (req, res, next) => {
		...
		let encoding = "identity"; try { encoding = new Negotiator(req).encoding(["gzip", "identity"]) ?? "identity"; } catch { /* malformed Accept-Encoding: serve identity */ }
...
var WebServer = class extends Service {
	...
	constructor(ctx, config) {
		...
		this.gzip = process.env.DSH_DESKTOP === "1" ? void 0 : resolved.compression === "gzip" ? createGzipMiddleware(resolved) : void 0;
	}
		...
		this.server = createServer((req, res) => {
			const next = () => {
					handle(req, res).catch((err) => {
						...
						console.error("WEB_SERVER_REQUEST_ERROR", err?.stack ?? err);
						if (process.env.DSH_DESKTOP === "1") { res.writeHead(500, { "content-type": "text/plain; charset=utf-8" }); res.end(String(err?.stack ?? err)); return; }
					if (res.headersSent) {
						res.destroy();
						return;
					}
